...
<br> the trick here is to transform this touch command into a usefull RCE ! there is much to try in this case but one of the ideas is to set the user as ``` user= "<username> & cat ./flag > ./file/username" ``` so the running command will be ``` touch ./files/<username> & cat ./flag > ./file/username ``` and once we set the token and send that request, abra kadabra. now the flag is written in our file in the system !
